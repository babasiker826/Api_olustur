...
# Kullanıcı API'lerini saklamak için sözlük
USER_APIS = {}
...
def generate_api_key(length=6):
    """6 haneli rakamlardan oluşan API key oluştur"""
...
def home():
    ...
    return Response(
        json.dumps({
            ...
            "create_your_own": "/apiolustur?name=API_ADINIZ"
        }, ensure_ascii=False, indent=2),
        content_type="application/json; charset=utf-8"
    )

@app.route("/apiolustur", methods=["GET"])
def create_api():
    """Kullanıcıya özel API oluştur"""
    ...
    if not name:
        return Response(
            json.dumps({
                "ok": False,
                "error": "API adı belirtmelisiniz. Örnek: /apiolustur?name=benimapi"
            }, ensure_ascii=False, indent=2),
            content_type="application/json; charset=utf-8",
            status=400
        )
    
    # Benzersiz 6 haneli API key oluştur
    while True:
        api_key = generate_api_key(6)
        if api_key not in USER_APIS:
            break
    
    # Kullanıcı API'sini kaydet
    USER_APIS[api_key] = {
        "name": name,
        "created_at": "2024",  # Gerçek uygulamada timestamp kullanın
        "endpoints": {}
    }
    ...
    return Response(
        json.dumps({
            "ok": True,
            "message": f"API'niz başarıyla oluşturuldu!",
            "api_name": name,
            "api_key": api_key,
            "your_base_url": f"/{name}",
            "example_usage": f"/{name}/adsoyad?api_key={api_key}&ad=AHMET&soyad=YILMAZ",
            "available_endpoints": list(TARGET_APIS.keys())
        }, ensure_ascii=False, indent=2),
        content_type="application/json; charset=utf-8"
    )

@app.route("/<user_api>/<service>", methods=["GET", "POST"])
def user_api_service(user_api, service):
    """Kullanıcıya özel API endpoint'i"""
    # Kullanıcı API'sini kontrol et
    api_key = request.args.get("api_key")
    
    if not api_key or api_key not in USER_APIS:
        return Response(
            json.dumps({
                "ok": False,
                "error": "Geçersiz API key",
                "solution": "Önce /apiolustur?name=API_ADINIZ ile API oluşturun"
            }, ensure_ascii=False, indent=2),
            content_type="application/json; charset=utf-8",
            status=401
        )
    ...
    # API key'i parametrelerden çıkar (hedef API'ye gitmesin)
    ...
    return Response(
        json.dumps({
            "ok": True,
            "api_name": USER_APIS[api_key]["name"],
            "service": service,
